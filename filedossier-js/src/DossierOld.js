...
import { Table, Button } from 'semantic-ui-react'
    ...
    getDossier = (dossierKey, dossierPackage, dossierCode) => {
        ...
        this.dossierApi.getDossier(dossierKey, dossierPackage, dossierCode, this.getDossierCallback);
    }
...
//    }
    ...
    getDossierCallback = (error, data, response) => {
        if (error) {
            ...
            this.setState({dossier: {loading: false, value: response.body}});
        }
    }
    ...
    render() {
        ...
        if (loading) {
            return (<div className="fileDosser">
                Загрузка
            </div>);
